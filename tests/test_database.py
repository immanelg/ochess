...
async def test_database_service(session: AsyncSession) -> None:
    ...
    user_repo = service.UserRepository(session)
    game_repo = service.GameRepository(session)
    ...
    game = await game_repo.accept_invite(user_2.id, schemas.AcceptInviteDataReceive(game_id=game.id))
    ...
    assert game.status == GameStatus.resign
